...
void sys_tables(int mode = 0){
  ...
  const int nSys = 20;
  TString processName[nProcess] = { "twdr", "tt", "others"};
  TString processLabel[nProcess] = { "\\textbf{$tW$}", "\\textbf{$t \\bar{t}$}", "others"};
   TString systName[nSys] = { "Nominal", "JERsysDown" , "JERsysUp","JESsysDown" , "JESsysUp","PUsysDown" , "PUsysUp","SFsysDown" , "SFsysUp","METsysDown" , "METsysUp","TopMassDown" ,"TopMassUp","Q2Down" , "Q2Up","eleSFsysDown" , "eleSFsysUp","matchingDown" , "matchingUp", "tW DS"}; 
...
///// FOR TTBAR /////////
  ...
  for (int i=2; i < 8; i++){
    salida << cutLabel[i];
    for (int j = 1; j < nProcess-1; j++){ // only for twdr
 ...
 /////////////////////
  ...
  for (int i=2; i < 8; i++){
    salida << cutLabel[i];
    for (int j = 2; j < nProcess; j++){ // only for other
