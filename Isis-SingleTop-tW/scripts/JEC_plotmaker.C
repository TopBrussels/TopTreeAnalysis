...
void JEC_plotmaker(int mode = 0, int region = 0){
  ...
  sprintf(myRootFile2,"results/noJER_an_%dpb_%d.root", lumi, mode); // take output from looper
  ...
  const int nPlots = 1;
  TString processName[nProcess] =  { "twdr"};
  TString processTitle[nProcess] = { "twdr signal"};
  Color_t color[2] =        { kBlue,kRed};
  
  
  TString cutLabel[nPlots] =     {  "et_jet"};
  int rebinHisto[nPlots] =       {  4};
  TString cutTitle[nPlots] =     { "Energy of the jet (GEV)"};
 

  TString modeString[3] = {"0","1","2"};
 
  TString plotExtension = "noJER_dis_plot_"; // name of the plots
  ...
  TH1F* histo_zjets_NOSF;
   ...
   for(int iPlots = 0; iPlots< nPlots; iPlots++)
   {
     ...
     histo_zjets= (TH1F*) _file0->Get(cutLabel[iPlots]+ "_" + processName[0]);
       ...
       leg ->AddEntry(histo_zjets, "twdr signal" , "l");  
       leg ->AddEntry(histo_zjetsNOSF, "twdr signal - no reweighing" , "l"); 
