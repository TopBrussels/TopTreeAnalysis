...
ResolutionFit::ResolutionFit(string label)
{
  ...
  for(unsigned int iEta=0; iEta<nEtaBins_; iEta++){
    Char_t controlCanName[30], controlCanEtaName[30], controlCanThetaName[30], controlCanPhiName[30], controlCanRelName[30];
    ...
    for(unsigned int iSel=0; iSel<nPtBins_; iSel++){
      ...
      resRelHisto_[(iEta*nPtBins_)+iSel] = new TH1F("resRelHisto_"+label_+"_"+etaEtNumber,"resRelHisto_"+etaEtNumber,200,-1,1);
	...
	resRelHistoIncl_[iSel] = new TH1F("resRelHisto_"+label_+"_EtaIncl_"+etaEtNumber,"resRelHisto_"+label_+"_EtaIncl_"+etaEtNumber,200,-1,1);
      }
    }
  }
...
void ResolutionFit::Fill(TLorentzVector *lorentzVector, TLorentzVector *mcParticle)
{
  ...
  for(unsigned int iEta=0; iEta<nEtaBins_; iEta++){
    for(unsigned int iSel=0; iSel<nPtBins_; iSel++){
      if( tmpJet.Pt() > jetPtBinning_[iSel] && tmpJet.Pt() < jetPtBinning_[iSel+1] ){
	if(iEta == 0){
	  binCenterHistoIncl_[iSel]->Fill( tmpJet.Pt() );
	  resRelHistoIncl_[iSel]->Fill( (tmpJet.Et() - mcParticle->Et()) / tmpJet.Et() );
	}
        
	if( fabs(tmpJet.Eta()) >= towerBinning_[iEta] && fabs(tmpJet.Eta()) < towerBinning_[iEta+1] ){
	  binCenterHisto_[(iEta*nPtBins_)+iSel]->Fill( tmpJet.Pt() );
	  resHisto_[(iEta*nPtBins_)+iSel]->Fill( tmpJet.Et() - mcParticle->Et() );
	  ...
	  resRelHisto_[(iEta*nPtBins_)+iSel]->Fill( (tmpJet.Et() - mcParticle->Et()) / tmpJet.Et() );
	}
      }
    } //loop over pTbins
  } //loop over etaBins
...
void ResolutionFit::CalculateResolutions()
{
  ...
  TString ethname[11];
  TString ethnameeta[11];
  TString ethnametheta[11];
  TString ethnamephi[11];
  TString ethnamerel[11];
  ...
  Double_t sigma[11];
  Double_t sigmaErr[11];
  Double_t sigmaEta[11];
  Double_t sigmaErrEta[11];
  Double_t sigmaTheta[11];
  Double_t sigmaErrTheta[11];
  Double_t sigmaPhi[11];
  Double_t sigmaErrPhi[11];
  Double_t sigmaRel[11];
  Double_t sigmaErrRel[11];
  Double_t mean[11];
  Double_t meanErr[11];
  Double_t meanEta[11];
  Double_t meanErrEta[11];
  Double_t meanTheta[11];
  Double_t meanErrTheta[11];
  Double_t meanPhi[11];
  Double_t meanErrPhi[11];
  Double_t meanRel[11];
  Double_t meanErrRel[11];
  Double_t meanRelIncl[11];
  Double_t meanErrRelIncl[11];
  
  TH1F* controlHisto[11];
  TH1F* controlHistoEta[11];
  TH1F* controlHistoTheta[11];
  TH1F* controlHistoPhi[11];
  TH1F* controlHistoRel[11];
  TH1F* controlHistoRelIncl[11];
