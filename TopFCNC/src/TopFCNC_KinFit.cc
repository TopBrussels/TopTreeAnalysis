...
TopFCNC_KinFit::TopFCNC_KinFit(Dataset* dataset, ResolutionFit *resFitLeptons, ResolutionFit *resFitBJets, ResolutionFit *resFitQJets, ResolutionFit *resFitLightJets){
  ...
  if(topFCNC_Evt->isDiLeptonic()){
    ...
    do{
      if(verbose_)
        ...
        kinfit_->setVerbosity(fit_verbose_);
    ...
    topFCNC_Evt->SetQuark1FromW(TRootJet(jets[Permutation[2]]));
    topFCNC_Evt->SetQuark2FromW(TRootJet(jets[Permutation[3]]));
