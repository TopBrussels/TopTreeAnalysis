...
//    inputMonsters.push_back("Monsters/Nominal/KinFit_LightMonsters_TopMassDiff_TTbarJets_mass175_5_Nominal_SemiLep.root");
//    inputMonsters.push_back("Monsters/Nominal/KinFit_LightMonsters_TopMassDiff_TTbarJets_mass178_5_Nominal_SemiLep.root");
//    inputMonsters.push_back("Monsters/Nominal/KinFit_LightMonsters_TopMassDiff_TTbarJets_mass181_5_Nominal_SemiLep.root");
//    inputMonsters.push_back("Monsters/Nominal/KinFit_LightMonsters_TopMassDiff_TTbarJets_mass184_5_Nominal_SemiLep.root");
  }
  ...
  for(unsigned int iDataSet=0; iDataSet<inputMonsters.size(); iDataSet++)
  {
    ...
    {
      double newXS_SF = 172.025 / 157.5; //NNLO, see http://arxiv.org/pdf/1303.6254.pdf
      color = kRed+1;
...
//    if( dataSet->Name().find("ST_tChannel") == 0 ) color = kMagenta+2;
  ...
  MSPlot["M3"] = new MultiSamplePlot(datasetsSemiMu,"M3",50,0,1000,"M3","Nr. of events");
  ...
  MSPlot["PtTopM3"] = new MultiSamplePlot(datasetsSemiMu,"PtTopM3",150,0,450,"PtTopM3","Nr. of events");
  MSPlot["PtTTbar"] = new MultiSamplePlot(datasetsSemiMu,"PtTTbar",100,0,250,"PtTTbar","Nr. of events");
...
//      if( monster->runID() > 174000 ) continue;
      ...
      if( dataSetName.find("TTbarJets") == 0 || dataSetName.find("TT_") == 0 )
      {
        ...
        float genWeight = exp(0.199 - 0.00166*pTgenTop);
  ...
  //        if( chi2MTopFit < 99999 ) // 99999 means no mTopFit value was calculated, see FullKinFit.cc
          if( chi2MTopFit < maxChi2 )
          {
            ...
            if(iCombi == bestCombi)
            {
              ...
              MSPlot["etaTop"]->Fill((selectedJets[combi[0]]+selectedJets[combi[1]]+selectedJets[combi[2]]).Eta(),dataSet,true,monster->eventWeight()*lumiWeight);
              MSPlot["PtTop"]->Fill((selectedJets[combi[0]]+selectedJets[combi[1]]+selectedJets[combi[2]]).Pt(), dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["etaBjet"]->Fill(selectedJets[combi[2]].Eta(), dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["PtBjet"]->Fill(selectedJets[combi[2]].Pt(), dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["MinDRLightB"]->Fill(minDRLightB, dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["DRLightJets"]->Fill(selectedJets[combi[0]].DeltaR(selectedJets[combi[1]]), dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["MinChi2ndf_Fit"+leptonChargeIncl]->Fill(chi2MTopFit, dataSet, true, monster->eventWeight()*lumiWeight);
              MSPlot["mTop_Fit"+leptonChargeIncl]->Fill(mTopFit, dataSet, true, monster->eventWeight()*lumiWeight);
              ...
              if(doAllMSPlots)
              {
                ...
                MSPlot["mTop_Fit"+leptonCharge]->Fill(mTopFit, dataSet, true, monster->eventWeight()*lumiWeight);
                MSPlot["sigmaMtop_Fit"+leptonCharge]->Fill(sigmaMTopFit, dataSet, true, monster->eventWeight()*lumiWeight);
                MSPlot["MinChi2ndf_Fit"+leptonCharge]->Fill(chi2MTopFit, dataSet, true, monster->eventWeight()*lumiWeight);
                MSPlot["Prob_MinChi2_Fit"+leptonCharge]->Fill(TMath::Prob(chi2MTopFit, 1), dataSet, true, monster->eventWeight()*lumiWeight);
              }
            }
      ...
      if( mTop.size() > 0 ) // event still has at least one good kinFitted jet-combi
      {
        ...
        MSPlot["MET"]->Fill(MET.Pt(), dataSet, true, monster->eventWeight()*lumiWeight);
        ...
        MSPlot["nBtags"]->Fill(nBtags, dataSet, true, monster->eventWeight()*lumiWeight);
        MSPlot["leptonCharge"]->Fill(monster->leptonCharge(), dataSet, true, monster->eventWeight()*lumiWeight);
        ...
        MSPlot["PtTopM3"]->Fill(maxPt, dataSet, true, monster->eventWeight()*lumiWeight);
        ...
        MSPlot["HT"]->Fill(Ht, dataSet, true, monster->eventWeight()*lumiWeight);
        ...
        MSPlot["Mttbar"]->Fill((selectedJets[0]+selectedJets[1]+selectedJets[2]+selectedJets[3]+lepton+MET).M(), dataSet, true, monster->eventWeight()*lumiWeight);
