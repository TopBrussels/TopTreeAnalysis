...
#include "TopTreeAnalysis/WHelicities/interface/BTagCosThetaCalculation.h"
...
{
  string rootFileName = "LightStopSearch_output.root";
	
  clock_t start = clock();
	
  cout << "************************************************************" << endl;
  cout << " Beginning of the programme for the search of light stops ! " << endl;
  cout << "************************************************************" << endl;
  ...
  bool calculateTransferFunctions = true; // true will have only an effect if useMassesAndResolutions = false
  int isCorrectlyMatched = 0;
  int isNotCorrectlyMatched = 0;
  int NofSelectedEvents = 0;
	int NofSelectedTTbarEvents = 0;
	int NofSelectedSignalEvents = 0;
	int NofTrueSemiMuTTbar = 0;
	int NofTrueSemiMuSignal = 0;
  int Nof2Btags = 0;
  int unconditionalLoop = 0;
  int inChi2Loop = 0;
  int labelsChanged = 0;
  int allMCJetsMatchedCondition = 0;
  int leptBMatched = 0;
  int hadrBMatched = 0;
  int WjetsMatched = 0;
  ...
  //xml file
  string xmlFileName ="../config/myLightStopSearch.xml"; // can contain both electron and muon datasets at the same time!
  ...
  map<string,TH2F*> histo2D;
	...
	int PtMax = 340;
	int NofEtaBins = 2;
	histo2D["BtaggedJets"] = new TH2F("BtaggedJets", "Total number of btagged jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["BtaggedBJets"] = new TH2F("BtaggedBJets", "Total number of btagged b-jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["BtaggedCJets"] = new TH2F("BtaggedCJets", "Total number of btagged c-jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["BtaggedLightJets"] = new TH2F("BtaggedLightJets", "Total number of btagged light jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["TotalNofBJets"] = new TH2F("TotalNofBJets", "Total number of b-jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["TotalNofCJets"] = new TH2F("TotalNofCJets", "Total number of c-jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
	histo2D["TotalNofLightJets"] = new TH2F("TotalNofLightJets", "Total number of light jets", NofPtBins, PtMin, PtMax, NofEtaBins, 0, 2.4);
  ...
  map<string,MultiSamplePlot*> MSPlot;
	MSPlot["Simulation_pT_jet4"] = new MultiSamplePlot(datasets, "Simulation_pT_jet4", 26, 20, 150, "p_{T} (GeV)");
	MSPlot["Simulation_Nof_Selected_Jets"] = new MultiSamplePlot(datasets, "Simulation_Nof_Selected_Jets", 7, 2.5, 9.5, "Nb. of Jets");
	MSPlot["Simulation_Nof_Btagged_Jets"] = new MultiSamplePlot(datasets, "Simulation_Nof_Btagged_Jets", 6, -0.5, 5.5, "Nb. of b-tagged jets");
	MSPlot["Simulation_Highest_Btag_Value"] = new MultiSamplePlot(datasets, "Simulation_Highest_Btag_Value", 20, 0, 1, "Highest b-tag value");
	MSPlot["Simulation_Btag_Value_jet1"] = new MultiSamplePlot(datasets, "Simulation_Btag_Value_jet1", 20, 0, 1, "b-tag value of hardest jet");
	MSPlot["Simulation_pT_muon"] = new MultiSamplePlot(datasets, "Simulation_pT_muon", 30, 0, 300, "p_{T,#,#mu} (GeV)");
	MSPlot["Simulation_relIso_Muon"] = new MultiSamplePlot(datasets, "Simulation_relIso_Muon", 26, 0, 0.13, "relIso");
	MSPlot["Simulation_MET_Et"] = new MultiSamplePlot(datasets,"Simulation_MET_Et", 60, 0, 300, "E_{T}^{miss} (GeV)");
	MSPlot["Simulation_Ht_4leadingjets"] = new MultiSamplePlot(datasets,"Simulation_Ht_4leadingjets", 100, 100, 1100, "H_{T} (GeV)");
	
	MSPlot["Simulation+data_pT_jet4"] = new MultiSamplePlot(datasets, "Simulation+data_pT_jet4", 26, 20, 150, "p_{T} (GeV)");
	MSPlot["Simulation+data_Nof_Selected_Jets"] = new MultiSamplePlot(datasets, "Simulation+data_Nof_Selected_Jets", 7, 2.5, 9.5, "Nb. of Jets");
	MSPlot["Simulation+data_Nof_Btagged_Jets"] = new MultiSamplePlot(datasets, "Simulation+data_Nof_Btagged_Jets", 6, -0.5, 5.5, "Nb. of b-tagged jets");
	MSPlot["Simulation+data_Highest_Btag_Value"] = new MultiSamplePlot(datasets, "Simulation+data_Highest_Btag_Value", 20, 0, 1, "Highest b-tag value");
	MSPlot["Simulation+data_Btag_Value_jet1"] = new MultiSamplePlot(datasets, "Simulation+data_Btag_Value_jet1", 20, 0, 1, "b-tag value of hardest jet");
	MSPlot["Simulation+data_pT_muon"] = new MultiSamplePlot(datasets, "Simulation+data_pT_muon", 30, 0, 300, "p_{T,#,#mu} (GeV)");
	MSPlot["Simulation+data_relIso_Muon"] = new MultiSamplePlot(datasets, "Simulation+data_relIso_Muon", 26, 0, 0.13, "relIso");
	MSPlot["Simulation+data_MET_Et"] = new MultiSamplePlot(datasets,"Simulation+data_MET_Et", 60, 0, 300, "E_{T}^{miss} (GeV)");
	MSPlot["Simulation+data_Ht_4leadingjets"] = new MultiSamplePlot(datasets,"Simulation+data_Ht_4leadingjets", 100, 100, 1100, "H_{T} (GeV)");
  ...
  CutsSelecTableSemiEl.push_back(string(LabelNJets));
	...
	CutsSelecTableSemiEl.push_back("Missing $E_T$");
	CutsSelecTableSemiMu.push_back("$H_T$ cut");
	CutsSelecTableSemiEl.push_back("$H_T$ cut");
  ...
  cout << " Initialized LumiReWeighting stuff" << endl;
	...
	BTagWeightTools *bTool_M = new BTagWeightTools("BtagFiles/SFb-pt_payload_Moriond13.txt","CSVM");
	
	
	
	////////////////////////////////////////////
	//  Initialise Muon Trigger ScaleFactors  //
	////////////////////////////////////////////
  ...
  //////////////////////////////////////
  //  LHCO output file for MadWeight  //
  //////////////////////////////////////
	
	ofstream muPlusLHCO;
	ofstream muMinusLHCO;
	
	ofstream muPlusLHCOextended;
	ofstream muMinusLHCOextended;
	
	ofstream muPlusScaleFactor;
	ofstream muMinusScaleFactor;
  ...
  for (unsigned int d = 0; d < datasets.size (); d++) {
    ...
    /*if ( (d == 0 && useMassesAndResolutions) || d == 1 || d > 5) {
		 cout << "Skip dataset (for now)" << endl;
		 continue;
		 }*/
		
		
		////////////////////////////
		//	Initialise LHCO files	//
		////////////////////////////
		
		if(dataSetName.find("TT") == 0) {
			thisDataSet = "TTbar";
		}
		if(dataSetName.find("W_Jets_3") == 0) {
			thisDataSet = "W+3jets";
		}
		if(dataSetName.find("W_Jets_4") == 0) {
			thisDataSet = "W+4jets";
		}
		if(dataSetName.find("Z_Jets_3") == 0) {
			thisDataSet = "Z+3jets";
		}
		if(dataSetName.find("Z_Jets_4") == 0) {
			thisDataSet = "Z+4jets";
		}
		if(dataSetName.find("ST_t_t") == 0) {
			thisDataSet = "ST_t_t";
		}
		if(dataSetName.find("ST_t_tbar") == 0) {
			thisDataSet = "ST_t_tbar";
		}
		if(dataSetName.find("ST_tW_t") == 0) {
			thisDataSet = "ST_tW_t";
		}
		if(dataSetName.find("ST_tW_tbar") == 0) {
			thisDataSet = "ST_tW_tbar";
		}
		if(dataSetName.find("Data_Mu") == 0 || dataSetName.find("data_Mu") == 0 || dataSetName.find("DATA_Mu") == 0) {
			thisDataSet = "Data_Mu";
		}
		//if(dataSetName.find("Data_El") == 0 || dataSetName.find("data_El") == 0 || dataSetName.find("DATA_El") == 0) thisDataSet = "Data_El";
		if(dataSetName.find("NP") == 0) {
			thisDataSet = "Signal";
		}
		
		
		string pathLHCO = "LHCO_CSVM/";
		mkdir(pathLHCO.c_str(),0777);
		
		string strMuPlus = pathLHCO+"input_muPlus_"+thisDataSet+".lhco";
		muPlusLHCO.open(strMuPlus.c_str());
		string strMuMinus = pathLHCO+"input_muMinus_"+thisDataSet+".lhco";
		muMinusLHCO.open(strMuMinus.c_str());
		
		muPlusLHCO << "#</MGPGSCard> " << endl;
		muPlusLHCO << "#" << thisDataSet << endl;
		muPlusLHCO << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
		
		muMinusLHCO << "#</MGPGSCard> " << endl;
		muMinusLHCO << "#" << thisDataSet << endl;
		muMinusLHCO << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
		
		
		string pathLHCOextended = "LHCO_Extended_CSVM/";
		mkdir(pathLHCOextended.c_str(),0777);
		
		string strMuPlusExt = pathLHCOextended+"input_muPlus_"+thisDataSet+".lhco";
		muPlusLHCOextended.open(strMuPlusExt.c_str());
		string strMuMinusExt = pathLHCOextended+"input_muMinus_"+thisDataSet+".lhco";
		muMinusLHCOextended.open(strMuMinusExt.c_str());
		
		muPlusLHCOextended << "#</MGPGSCard> " << endl;
		muPlusLHCOextended << "#" << thisDataSet << endl;
		muPlusLHCOextended << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
		
		muMinusLHCOextended << "#</MGPGSCard> " << endl;
		muMinusLHCOextended << "#" << thisDataSet << endl;
		muMinusLHCOextended << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
		
		
		string strScaleFactor = pathLHCOextended+"scaleFactor_muPlus_"+thisDataSet+".lhco";
		muPlusScaleFactor.open(strScaleFactor.c_str());
		muPlusScaleFactor << "#" << thisDataSet << endl;
		strScaleFactor = pathLHCOextended+"scaleFactor_muMinus_"+thisDataSet+".lhco";
		muMinusScaleFactor.open(strScaleFactor.c_str());
		muMinusScaleFactor << "#" << thisDataSet << endl;
		
		
		int ProcessedLHCOEventsMuPlus = 0;  // Counts the number of events for the LHCO file
		int ProcessedLHCOEventsMuMinus = 0;
		int MWSameDataSetNumberMuPlus = 1;
		int MWSameDataSetNumberMuMinus = 1;
		
		
		
    /////////////////////////////////////
    /// Initialise JEC factors
    /////////////////////////////////////
    ...
    float SigmaChi2Topmass = -9999;
		...
		// initialise histos for btag scaleFactors
		TH2F *inBtaggedJets, *inBtaggedBJets, *inBtaggedCJets, *inBtaggedLightJets, *inTotalNofBJets, *inTotalNofCJets, *inTotalNofLightJets;
    ...
    if (useMassesAndResolutions) {
      ...
      delete res;
			...
			// Btag histos
			string fileNameBtag = "PlotsForBtagWeights.root";
			TFile *fileIn = new TFile(fileNameBtag.c_str(),"READ");
			fileIn->cd();
			inBtaggedJets  = (TH2F*) fileIn->Get("BtaggedJets");
			inBtaggedBJets = (TH2F*) fileIn->Get("BtaggedBJets");
			inBtaggedCJets = (TH2F*) fileIn->Get("BtaggedCJets");
			inBtaggedLightJets = (TH2F*) fileIn->Get("BtaggedLightJets");
			inTotalNofBJets = (TH2F*) fileIn->Get("TotalNofBJets");
			inTotalNofCJets = (TH2F*) fileIn->Get("TotalNofCJets");
			inTotalNofLightJets = (TH2F*) fileIn->Get("TotalNofLightJets");
			...
			//for (unsigned int ievt = 0; ievt < 20000; ievt++)
    {
			...
			if(dataSetName.find("TTbarJets") == 0)  {
	  		...
	  		//cout << "is semimu? " << genEvt->isSemiLeptonic(TRootGenEvent::kMuon) << endl;
			...
			if (triggedSemiMu){
	  		...
	  		if(isGoodPV) {
	    		...
	    		if (selectedMuons.size() == 1) {
						//-----------------------------//
						//  Muon Trigger Scalefactors  //
						//-----------------------------//
						
						if ( ! (dataSetName.find("Data") == 0 || dataSetName.find("data") == 0 || dataSetName.find("DATA") == 0 ) ) {
							double muonTrigSF = leptonTools->getMuonSF(selectedMuons[0]->Eta(), selectedMuons[0]->Pt());
				 			...
				 			if (vetoElectronsSemiMu.size() == 0) {
		  		 			...
		  		 			if( selectedJets.size()>=4 ) {
									...
									for (unsigned int i=0; i < selectedJets.size(); i++) {
										if (fabs(selectedJets[i]->Eta()) > 2.4) {
											correctEta = false;
											cout << "WARNING: Jet has eta larger than 2.4. Event " << ievt << " will not be processed!" << endl;
											break;
										}
									}
		    		 			if (useMassesAndResolutions && correctEta) selecTableSemiMu.Fill(d,6,scaleFactor);
									
									//nb. of b-tags
									float maxBTagValue = 0;
									for (unsigned int i=0; i < selectedJets.size(); i++) {
										if (selectedJets[i]->btag_combinedSecondaryVertexBJetTags() > 0.679) {
											nb_btags++;
										}
										if (selectedJets[i]->btag_combinedSecondaryVertexBJetTags() > maxBTagValue) {
											maxBTagValue = selectedJets[i]->btag_combinedSecondaryVertexBJetTags();
										}
									}
									
									//fill plots
									if (correctEta) {
										MSPlot["Simulation+data_pT_jet4"]->Fill(selectedJets[3]->Pt(), datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_Nof_Selected_Jets"]->Fill(selectedJets.size(), datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_Nof_Btagged_Jets"]->Fill(nb_btags, datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_Highest_Btag_Value"]->Fill(maxBTagValue, datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_Btag_Value_jet1"]->Fill(selectedJets[0]->btag_combinedSecondaryVertexBJetTags(), datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_pT_muon"]->Fill(selectedMuons[0]->Pt(), datasets[d], true, Luminosity*scaleFactor);
										...
										MSPlot["Simulation+data_relIso_Muon"]->Fill(muon_relIso, datasets[d], true, Luminosity*scaleFactor);
										MSPlot["Simulation+data_MET_Et"]->Fill(mets[0]->Et(),datasets[d], true, Luminosity*scaleFactor);
										
										if ( ! (dataSetName.find("Data") == 0 || dataSetName.find("data") == 0 || dataSetName.find("DATA") == 0 )) {
											MSPlot["Simulation_pT_jet4"]->Fill(selectedJets[3]->Pt(), datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_Nof_Selected_Jets"]->Fill(selectedJets.size(), datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_Nof_Btagged_Jets"]->Fill(nb_btags, datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_Highest_Btag_Value"]->Fill(maxBTagValue, datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_Btag_Value_jet1"]->Fill(selectedJets[0]->btag_combinedSecondaryVertexBJetTags(), datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_pT_muon"]->Fill(selectedMuons[0]->Pt(), datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_relIso_Muon"]->Fill(muon_relIso, datasets[d], true, Luminosity*scaleFactor);
											MSPlot["Simulation_MET_Et"]->Fill(mets[0]->Et(),datasets[d], true, Luminosity*scaleFactor);
											}										
										
									}
									
									if (mets[0]->Et() > 70) {
										if (useMassesAndResolutions && correctEta) selecTableSemiMu.Fill(d,7,scaleFactor);
										float Ht = selectedJets[0]->Pt()+selectedJets[1]->Pt()+selectedJets[2]->Pt()+selectedJets[3]->Pt();
										if (correctEta && ! (dataSetName.find("Data") == 0 || dataSetName.find("data") == 0 || dataSetName.find("DATA") == 0 )) {
											MSPlot["Simulation_Ht_4leadingjets"]->Fill(Ht, datasets[d], true, Luminosity*scaleFactor);
										}
										if (correctEta) {
											MSPlot["Simulation+data_Ht_4leadingjets"]->Fill(Ht, datasets[d], true, Luminosity*scaleFactor);
										}
										if (Ht > 250) {
											if (useMassesAndResolutions && correctEta) selecTableSemiMu.Fill(d,8,scaleFactor);
											
		    		 			 		if( nb_btags>=1 ) {
												// Apply btag scaleFactors
												if ( useMassesAndResolutions && correctEta && ! (dataSetName.find("Data") == 0 || dataSetName.find("data") == 0 || dataSetName.find("DATA") == 0 ) ) {
													if (MSPlot.find("BtagWeights") == MSPlot.end()){
	  												MSPlot["BtagWeights"] = new MultiSamplePlot(datasets, "BtagWeights", 50, 0.5, 1.5, "Weight");
														MSPlot["BtagWeightsOverkill"] = new MultiSamplePlot(datasets, "BtagWeightsOverkill", 75, 0.5, 2, "Weight");
													}
													...
													for (unsigned int i=0; i < selectedJets.size(); i++) {
														int xBin = 0;
														if (selectedJets[i]->Pt() < PtMax) {
															xBin = inTotalNofBJets->GetXaxis()->FindBin(selectedJets[i]->Pt());		//Histograms have same binning.
														}
														else if (selectedJets[i]->Pt() >= PtMax) {
															xBin = inTotalNofBJets->GetXaxis()->FindBin(PtMax-1);
														}
														int yBin = 0;
														if (fabs(selectedJets[i]->Eta()) < 2.4) {
															yBin = inTotalNofBJets->GetYaxis()->FindBin(fabs(selectedJets[i]->Eta()));
														}
														else if (fabs(selectedJets[i]->Eta()) == 2.4) {
															yBin = inTotalNofBJets->GetYaxis()->FindBin(2.4-0.01);
														}
														
														
														if (fabs(selectedJets[i]->partonFlavour()) == 5.) {
															double NofBJets = inTotalNofBJets->GetBinContent(xBin,yBin);
															if (NofBJets == 0.) {
																cout << "WARNING: No b jets for bin (" << xBin << "," << yBin << ")." << endl;
																continue;
															}
															double NofTaggedBJets = inBtaggedBJets->GetBinContent(xBin,yBin);
															tagEff = NofTaggedBJets/NofBJets;
															btagSF = bTool_M->getWeight(selectedJets[i]->Pt(), selectedJets[i]->Eta(), 5, "CSVM", 0);
														}
														else if (fabs(selectedJets[i]->partonFlavour()) == 4.) {
															double NofCJets = inTotalNofCJets->GetBinContent(xBin,yBin);
															if (NofCJets == 0.) {
																cout << "WARNING: No c jets for bin (" << xBin << "," << yBin << ")." << endl;
																continue;
															}
															double NofTaggedCJets = inBtaggedCJets->GetBinContent(xBin,yBin);
															tagEff = NofTaggedCJets/NofCJets;
															btagSF = bTool_M->getWeight(selectedJets[i]->Pt(), selectedJets[i]->Eta(), 4, "CSVM", 0);
														}
														else if (fabs(selectedJets[i]->partonFlavour()) == 1. || fabs(selectedJets[i]->partonFlavour()) == 2. || fabs(selectedJets[i]->partonFlavour()) == 3. || fabs(selectedJets[i]->partonFlavour()) == 21.) {
															double NofLightJets = inTotalNofLightJets->GetBinContent(xBin,yBin);
															if (NofLightJets == 0.) {
																cout << "WARNING: No light jets for bin (" << xBin << "," << yBin << ")." << endl;
																continue;
															}
															double NofTaggedLightJets = inBtaggedLightJets->GetBinContent(xBin,yBin);
															tagEff = NofTaggedLightJets/NofLightJets;
															btagSF = bTool_M->getWeight(selectedJets[i]->Pt(), selectedJets[i]->Eta(), 1, "CSVM", 0);
														}
															...
															cout << endl;
															cout << "WARNING: Jet not identified! PdgId: " << selectedJets[i]->partonFlavour() << endl;
															continue;
														}
														...
														if (tagEff == 0.) {
															cout << endl;
															cout << "WARNING: Tag efficiency is zero!" << endl;
															continue;
														}
														if (btagSF == 0.) {
															cout << endl;
															cout << "WARNING: Btag scalefactor is zero!" << endl;
															continue;
														}
														else if (btagSF*tagEff >= 1.) {
															cout << endl;
															cout << "WARNING:	BtagSF*tagEff is larger than 1!" << endl;
														}
													...
													double BtagWeight = probData/probMC;
													MSPlot["BtagWeights"]->Fill(BtagWeight, datasets[d], true, Luminosity*scaleFactor);
													MSPlot["BtagWeightsOverkill"]->Fill(BtagWeight, datasets[d], true, Luminosity*scaleFactor);
													scaleFactor = scaleFactor*BtagWeight;
												}
		      		 			 		if (useMassesAndResolutions && correctEta) selecTableSemiMu.Fill(d,9,scaleFactor);
												
		      		 			 		eventselectedSemiMu_onebtag = true;
												
		      		 			 		if( nb_btags>=2 ) {
					 						 		if (useMassesAndResolutions && correctEta) selecTableSemiMu.Fill(d,10,scaleFactor);
			...
			// Fill histos
			if (!useMassesAndResolutions && correctEta) {
				for (unsigned int i=0; i < selectedJets.size(); i++) {
					double localPt = selectedJets[i]->Pt();
					double localEta = fabs(selectedJets[i]->Eta());
					if (localPt >= PtMax) localPt = PtMax-1;
					if (localEta == 2.4) localEta = 2.4-0.01;
					
					if (fabs(selectedJets[i]->partonFlavour()) == 5.) {	//b-jet
						histo2D["TotalNofBJets"]->Fill(localPt,localEta);
					}
					else if (fabs(selectedJets[i]->partonFlavour()) == 4.) {	//c-jet
						histo2D["TotalNofCJets"]->Fill(localPt,localEta);
					}
						...
						histo2D["TotalNofLightJets"]->Fill(localPt,localEta);
					}
					else if (fabs(selectedJets[i]->partonFlavour()) == 0.) {
						cout << "Eventnr.: " << ievt << ", Jetnr.: " << i << ", pdgId: " << selectedJets[i]->partonFlavour() << ". Is pileup jet -> consider as light jet." << endl;
						histo2D["TotalNofLightJets"]->Fill(localPt,localEta);
					...
					if (selectedJets[i]->btag_combinedSecondaryVertexBJetTags() > 0.679) {	//CSVM
						histo2D["BtaggedJets"]->Fill(localPt,localEta);
						
						if (fabs(selectedJets[i]->partonFlavour()) == 5.) {	//b-jet
							histo2D["BtaggedBJets"]->Fill(localPt,localEta);
						}
						else if (fabs(selectedJets[i]->partonFlavour()) == 4.) {	//c-jet
							histo2D["BtaggedCJets"]->Fill(localPt,localEta);
						}
						else if (fabs(selectedJets[i]->partonFlavour()) == 1. || fabs(selectedJets[i]->partonFlavour()) == 2. || fabs(selectedJets[i]->partonFlavour()) == 3. || fabs(selectedJets[i]->partonFlavour()) == 21.) {	//udsg-jet
							histo2D["BtaggedLightJets"]->Fill(localPt,localEta);
						}
						else if (fabs(selectedJets[i]->partonFlavour()) == 0.) {
							cout << "Eventnr.: " << ievt << ", Jetnr.: " << i << ", pdgId: " << selectedJets[i]->partonFlavour() << ". Is pileup jet -> consider as light jet." << endl;
							histo2D["BtaggedLightJets"]->Fill(localPt,localEta);
			...
			if (/*!useMassesAndResolutions &&*/ dataSetName.find("TTbarJets") == 0 && (isSemiMu || isSemiEl) && correctEta) {
			...
			if (all4JetsMatched_MCdef_ && !useMassesAndResolutions && dataSetName.find("TTbarJets") == 0 && correctEta) {
	  		//cout << "Size vector: " << mcParticlesMatching_.size() << endl;
			...
			bool isSemiMuSignal = false;
			
			if (useMassesAndResolutions && correctEta) {
				if (dataSetName.find("TTbar") == 0) {
					NofSelectedTTbarEvents++;
					if (isSemiMu) NofTrueSemiMuTTbar++;
				}
				if (dataSetName.find("NP") == 0) {
					for (unsigned int i=0; i<mcParticles.size(); i++) {
						if ( mcParticles[i]->status() != 3) continue;
						if ( (mcParticles[i]->type() == 13 && mcParticles[i]->motherType() == -24 && mcParticles[i]->grannyType() == -6) || (mcParticles[i]->type() == -13 && mcParticles[i]->motherType() == 24 && mcParticles[i]->grannyType() == 6) ) {
							isSemiMuSignal = true;
						}
					}
					NofSelectedSignalEvents++;
					if (isSemiMuSignal) NofTrueSemiMuSignal++;
				}
			}
			...
			if (MSPlot.find("Selected_Events_pT_jet1"+Flav) == MSPlot.end()){
	  		...
	  		MSPlot["NofSelectedJets"+Flav] = new MultiSamplePlot(datasetsPlot, "NofSelectedJets"+Flav, 10, 3.5, 13.5, "Nb. of Jets");
	  		MSPlot["Selected_Events_Btag_Values"+Flav] = new MultiSamplePlot(datasetsPlot, "Selected_Events_Btag_Values"+Flav, 30, -1, 1.1, "b-tag value");
	  		MSPlot["Selected_Events_nb_Btags"+Flav] = new MultiSamplePlot(datasetsPlot, "Selected_Events_nb_Btags"+Flav, 5, 1.5, 6.5, "Nb. of b-tagged jets");
	  		MSPlot["NofPV"] = new MultiSamplePlot(datasetsPlot, "NofPV", 41, -0.5, 40.5, "Nb. of primary vertices");
	  		MSPlot["NofPV_after_lumiWeight"] = new MultiSamplePlot(datasetsPlot, "NofPV_after_lumiWeight", 41, -0.5, 40.5, "Nb. of primary vertices");
	  		MSPlot["Pileup_Reweighting"+Flav] = new MultiSamplePlot(datasetsPlot,"Pileup_Reweighting"+Flav, 40, 0, 2, "lumiWeight");
	  		MSPlot["MET_Pt"+Flav] = new MultiSamplePlot(datasetsPlot,"MET_Pt"+Flav, 80,60, 460, "p_{T} (GeV)");
	  		MSPlot["MET_Et"+Flav] = new MultiSamplePlot(datasetsPlot,"MET_Et"+Flav, 80, 60, 460, "E_{T} (GeV)");
	  		MSPlot["Ht_4leadingjets"+Flav] = new MultiSamplePlot(datasetsPlot,"Ht_4leadingjets"+Flav, 80, 250, 1050, "H_{T} (GeV)");
				MSPlot["Ht+MEt"+Flav] = new MultiSamplePlot(datasetsPlot,"Ht+MEt"+Flav, 100, 320, 1120, "H_{T}+E_{T}^{miss} (GeV)");
				MSPlot["Ht+MEt"+Flav+"_1000"] = new MultiSamplePlot(datasetsPlot,"Ht+MEt"+Flav+"_1000", 1000, 320, 1120, "H_{T}+E_{T}^{miss} (GeV)");
				MSPlot["Ht+MEt+Pt"+Flav] = new MultiSamplePlot(datasetsPlot,"Ht+MEt+Pt"+Flav, 100, 340, 1140, "H_{T}+E_{T}^{miss}+P_{T,#mu} (GeV)");
				MSPlot["Ht+MEt+Pt"+Flav+"_1000"] = new MultiSamplePlot(datasetsPlot,"Ht+MEt+Pt"+Flav+"_1000", 1000, 340, 1140, "H_{T}+E_{T}^{miss}+P_{T,#mu} (GeV)");
	  		...
	  		MSPlot["Chi2_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"Chi2_2btags"+Flav, 150, 0, 75, "{#chi}^2");
				MSPlot["W_Mass_2btags"+Flav] = new MultiSamplePlot(datasetsPlot, "W_Mass_2btags"+Flav, 100, 0, 450, "W Mass (GeV)");
	  		MSPlot["hadTop_Ht_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"hadTopHt_2btags"+Flav, 90, 60, 960, "H_{T} (GeV)");
	  		MSPlot["hadTop_Mass_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"hadTop_Mass_2btags"+Flav, 90, 60, 960, "Top Mass (GeV)");
	  		MSPlot["hadTop_Pt_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"hadTop_Pt_2btags"+Flav, 100, 0, 600, "Mass (GeV)");
	  		MSPlot["TTbar_Mass_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"TTbar_Mass_2btags"+Flav, 100, 200, 1200, "Mass (GeV)");
	  		MSPlot["TTbar_Angle_2btags"+Flav] = new MultiSamplePlot(datasetsPlot,"TTbar_Angle_2btags"+Flav, 70, 0, 7, "Phi");
				
	  		if (eventselectedSemiMu_onebtag) {
	    		MSPlot["Selected_Events_d0_Muon"] = new MultiSamplePlot(datasetsPlot, "Selected_Events_d0_Muon", 28, 0, 0.07, "d0");
	    		MSPlot["Selected_Events_relIso_Muon"] = new MultiSamplePlot(datasetsPlot, "Selected_Events_relIso_Muon", 26, 0, 0.13, "relIso");
	  		}
	  		...
	  		MSPlot["Check_Correct_Match"] = new MultiSamplePlot(datasetsPlot, "Check_Correct_Match", 4, -1.5, 2.5, "is correct match");
	  		MSPlot["Cos_Theta*"] = new MultiSamplePlot(datasetsPlot, "Cos_Theta*", 30, -1.5, 1.5, "Cos Theta*");
			...
			if (useMassesAndResolutions && correctEta) {
	  		...
	  		MSPlot["Ht_4leadingjets"+Flav]->Fill(HT, datasets[d], true, Luminosity*scaleFactor);
				float jetAndMEt = HT + mets[0]->Et();
				MSPlot["Ht+MEt"+Flav]->Fill(jetAndMEt, datasets[d], true, Luminosity*scaleFactor);
				MSPlot["Ht+MEt"+Flav+"_1000"]->Fill(jetAndMEt, datasets[d], true, Luminosity*scaleFactor);
				float jetAndMEtAndPtMu = jetAndMEt + selectedMuons[0]->Pt();
				MSPlot["Ht+MEt+Pt"+Flav]->Fill(jetAndMEtAndPtMu, datasets[d], true, Luminosity*scaleFactor);
				MSPlot["Ht+MEt+Pt"+Flav+"_1000"]->Fill(jetAndMEtAndPtMu, datasets[d], true, Luminosity*scaleFactor);
			...
			/* we require at least two b-tags in the event selection, hence this part does not make much sense
			 ...
			 if (ijet != kjet && ijet != labelBtag1 && jjet != kjet && jjet != labelBtag1 && kjet != labelBtag1) {
			 ...
			 float hadtopmass_1btag = ( *selectedJets[labelsReco[1]] + *selectedJets[labelsReco[2]] +  *selectedJets[labelsReco[3]]).M();
			 float hadtoppt_1btag = ( *selectedJets[labelsReco[1]] + *selectedJets[labelsReco[2]] +  *selectedJets[labelsReco[3]]).Pt();
			 MSPlot["hadTop_Mass_1btag"+Flav]->Fill(hadtopmass_1btag, datasets[d], true, Luminosity*scaleFactor);
			 MSPlot["hadTop_Pt_1btag"+Flav]->Fill(hadtoppt_1btag, datasets[d], true, Luminosity*scaleFactor);
			 ...
			 //float TTbarMass_1btag = leptonicTopMass_1btag + hadtopmass_1btag;
			...
			if (useMassesAndResolutions && labelBtag1 != -9999 && labelBtag2 != -9999) {
	  		...
	  		for (int ijet=0; ijet<4; ijet++) {
	    		for (int jjet=ijet+1; jjet<4; jjet++) {
	      		unconditionalLoop++;
	      		if (ijet != labelBtag1 && ijet != labelBtag2 && jjet != labelBtag1 && jjet != labelBtag2) {
							inChi2Loop++;
			...
			if (useMassesAndResolutions && all4JetsMatched_MCdef_) {
	  		allMCJetsMatchedCondition++;
	  		if (labelsReco[0] == MCPermutation[3] && labelsReco[1] == MCPermutation[2] && ( (labelsReco[2] == MCPermutation[0] && labelsReco[3] == MCPermutation[1]) || (labelsReco[2] == MCPermutation[1] && labelsReco[3] == MCPermutation[0]) )) {
	    		isCorrectMatch = true;
	    		isCorrectlyMatched++;
	  		}
	  		else {
	    		isNotCorrectlyMatched++;
	  		}
	  		MSPlot["Check_Correct_Match"]->Fill(isCorrectMatch, datasets[d], true, Luminosity*scaleFactor);
				
	  		if (labelsReco[0]==MCPermutation[3]) leptBMatched++;
	  		if (labelsReco[1]==MCPermutation[2]) hadrBMatched++;
	  		if ((labelsReco[2]==MCPermutation[0] && labelsReco[3]==MCPermutation[1]) || (labelsReco[2]==MCPermutation[1] && labelsReco[3]==MCPermutation[0])) WjetsMatched++;
				
	  		/*cout << "      Leptonic b / Hadronic b / W jets" << endl;
				 cout << "MC:   " << MCPermutation[3] << " " << MCPermutation[2] << " " << MCPermutation[0] << " " << MCPermutation[1] << endl;
				 cout << "Other: " << labelsReco[0] << " " << labelsReco[1] << " " << labelsReco[2] << " " << labelsReco[3] << endl;*/
			}
			
			
			
			//-----------------------------------------//
			// Create .lhco file for MadWeight testing //
			//-----------------------------------------//
			
			if (useMassesAndResolutions) {		//Events with jet->Eta() > 2.4 need to be in here!! (Otherwise weights of already processed lhco files won't match.)
				
				cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
				
				TLorentzVector LHCOEventsVector[6];
				
				double leptonCharge = 0;
				TRootMuon* muon = (TRootMuon*) selectedMuons[0];
				if(eventselectedSemiMu) leptonCharge = muon->charge();
				//if (d>0) cout << "lepton charge: " << leptonCharge << endl;
				
				if (leptonCharge > 0. && eventselectedSemiMu) {	//Currently looking at muPlus
					LHCOEventsVector[0] = (TLorentzVector) *selectedJets[labelsReco[0]];
					LHCOEventsVector[1] = (TLorentzVector) *selectedMuons[0];
					LHCOEventsVector[2] = (TLorentzVector) *mets[0];
					LHCOEventsVector[3] = (TLorentzVector) *selectedJets[labelsReco[1]];
					LHCOEventsVector[4] = (TLorentzVector) *selectedJets[labelsReco[2]];
					LHCOEventsVector[5] = (TLorentzVector) *selectedJets[labelsReco[3]];
					
					
					//Make LHCO file with all info
					for(int ii = 0; ii < 6; ii++){ //Loop over the different TLorentzVectors inside the array
						double EtaValue = LHCOEventsVector[ii].Eta();
						if(EtaValue == -0) EtaValue = 0.0;
						double PhiValue = LHCOEventsVector[ii].Phi();
						if(PhiValue < -3.14155) PhiValue = 3.1416;
						double MassValue = LHCOEventsVector[ii].M();
						if(MassValue < 0.001) MassValue = 0.0; //Avoid having masses with e-07
						int MadGraphId;
						if(ii == 0 || ii == 3 || ii == 4 || ii == 5) MadGraphId = 4;  //Jets
						else if(ii == 1) MadGraphId = 2; //Lepton (1 for electron case)
						else if(ii == 2) MadGraphId = 6; //MET
						
						
						if(ii == 0) {
							muPlusLHCOextended << "  0             " << ProcessedLHCOEventsMuPlus << "        0 " << "        " << event->runId() << "        " << event->eventId() << "        " << event->nb();
							cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
							cout.precision(3);
							muPlusLHCOextended << "        " << fixed << setprecision(3) << scaleFactor << endl;
						}  //Start of a new event
						muPlusLHCOextended << "  " << setprecision(1) << ii+1;		// setprecision(1) --> set maximum number of meaningful digits to 1. When 'fixed' or 'scientific', only count after decimal point.
						muPlusLHCOextended << "    " << setprecision(1) << MadGraphId;
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(4);
						muPlusLHCOextended << "  " << fixed << showpoint << setprecision(4) << EtaValue;		// showpoint --> also when decimal part is zero
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(4);
						muPlusLHCOextended << "  " << fixed << showpoint << setprecision(4) << PhiValue;
						muPlusLHCOextended << "  " << setprecision(4) << LHCOEventsVector[ii].Pt();
						cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
						cout.precision(4);
						muPlusLHCOextended << "  " << fixed << showpoint << setprecision(3) << MassValue;
						if(ii == 1) muPlusLHCOextended << "   " << setprecision(3) << "1.000";
						else muPlusLHCOextended << "    0.00";
						if (ii == 0 || ii == 3) muPlusLHCOextended << "  1.000"; //Btag
						else muPlusLHCOextended << "  0.00";
						muPlusLHCOextended << "     0.00  0.00  0.00" << endl;
					}
					
					
					//Make LHCO file for MadWeight
					for (unsigned int qW = 0; qW < 2; qW++) { //Add event twice, with W-jets interchanged
						if (qW == 1) {
							LHCOEventsVector[4] = (TLorentzVector) *selectedJets[labelsReco[3]];
							LHCOEventsVector[5] = (TLorentzVector) *selectedJets[labelsReco[2]];
						}
						...
						for(int ii = 0; ii < 6; ii++){ //Loop over the different TLorentzVectors inside the array
							...
							if(ii == 0) muPlusLHCO << "  0             " << ProcessedLHCOEventsMuPlus << "        0 " << endl;  //Start of a new event
							muPlusLHCO << "  " << setprecision(1) << ii+1;		// setprecision(1) --> set maximum number of meaningful digits to 1. When 'fixed' or 'scientific', only count after decimal point.
							muPlusLHCO << "    " << setprecision(1) << MadGraphId;
							cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
							cout.precision(4);
							muPlusLHCO << "  " << fixed << showpoint << setprecision(4) << EtaValue;		// showpoint --> also when decimal part is zero
							cout.setf(ios::fixed,ios::floatfield);
							cout.precision(4);
							muPlusLHCO << "  " << fixed << showpoint << setprecision(4) << PhiValue;
							muPlusLHCO << "  " << setprecision(4) << LHCOEventsVector[ii].Pt();
							cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
							cout.precision(4);
							muPlusLHCO << "  " << fixed << showpoint << setprecision(3) << MassValue;
							if(ii == 1) muPlusLHCO << "   " << setprecision(3) << "1.000";
							else muPlusLHCO << "    0.00";
							if (ii == 0 || ii == 3) muPlusLHCO << "  1.000"; //Btag
							else muPlusLHCO << "  0.00";
							muPlusLHCO << "     0.00  0.00  0.00" << endl;
						}
					}
					ProcessedLHCOEventsMuPlus++;
					//Only 2000 events per MadWeight LHCO file
					if (ProcessedLHCOEventsMuPlus%1000 == 0) {
						muPlusLHCO.close();
						ostringstream ostr;
						ostr << MWSameDataSetNumberMuPlus;
						string newFileNumber = ostr.str();
						string strMuPlusNew = pathLHCO+"input_muPlus_"+thisDataSet+newFileNumber+".lhco";
						muPlusLHCO.open(strMuPlusNew.c_str());
						muPlusLHCO << "#</MGPGSCard> " << endl;
						muPlusLHCO << "#" << thisDataSet << endl;
						muPlusLHCO << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
						
						MWSameDataSetNumberMuPlus++;
					}
					
					//scaleFactor file
					muPlusScaleFactor << event->eventId() << "    ";
					if (correctEta) {
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(3);
						muPlusScaleFactor << fixed << setprecision(3) << scaleFactor;
					}
					else {
						muPlusScaleFactor << "0.000";
					}
					if (isCorrectMatch) {
						muPlusScaleFactor << "    " << "1.00" << endl;
					}
					else {
						muPlusScaleFactor << "    " << "0.00" << endl;
					}
				}
				
				if(leptonCharge < 0. && eventselectedSemiMu){	//Currently looking at muMinus
					LHCOEventsVector[0] = (TLorentzVector) *selectedJets[labelsReco[1]];
					LHCOEventsVector[1] = (TLorentzVector) *selectedJets[labelsReco[2]];
					LHCOEventsVector[2] = (TLorentzVector) *selectedJets[labelsReco[3]];
					LHCOEventsVector[3] = (TLorentzVector) *selectedJets[labelsReco[0]];
					LHCOEventsVector[4] = (TLorentzVector) *selectedMuons[0];
					LHCOEventsVector[5] = (TLorentzVector) *mets[0];
					...
					for(int ii = 0; ii < 6; ii++){ //Loop over the different TLorentzVectors inside the array
						...
						if(MassValue < 0.001) MassValue = 0.0; //Avoid having masses with e-07
						int MadGraphId;
						if(ii == 0 || ii == 1 || ii == 2 || ii == 3) MadGraphId = 4;  //Jets
						else if(ii == 4) MadGraphId = 2; //Lepton (1 for electron case)
						else if(ii == 5) MadGraphId = 6; //MET
						
						
						if(ii == 0) {
							muMinusLHCOextended << "  0             " << ProcessedLHCOEventsMuMinus << "        0 " << "        " << event->runId() << "        " << event->eventId() << "        " << event->nb();
							cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
							cout.precision(3);
							muMinusLHCOextended << "        " << fixed << setprecision(3) << scaleFactor << endl;
						}  //Start of a new event
						muMinusLHCOextended << "  " << setprecision(1) << ii+1;
						muMinusLHCOextended << "    " << setprecision(1) << MadGraphId;
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(4);
						muMinusLHCOextended << "  " << fixed << showpoint << setprecision(4) << EtaValue;
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(4);
						muMinusLHCOextended << "  " << fixed << showpoint << setprecision(4) << PhiValue;
						muMinusLHCOextended << "  " << setprecision(4) << LHCOEventsVector[ii].Pt();
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(4);
						muMinusLHCOextended << "  " << fixed << showpoint << setprecision(3) << MassValue;
						if(ii == 4) muMinusLHCOextended << "   " << setprecision(3) << "-1.000";
						else muMinusLHCOextended << "    0.00";
						if (ii == 0 || ii == 3) muMinusLHCOextended << "  1.000"; //Btag
						else muMinusLHCOextended << "  0.00";
						muMinusLHCOextended << "     0.00  0.00  0.00" << endl;
					}
					
					
					//Make LHCO file for MadWeight
					for (unsigned int qW = 0; qW < 2; qW++) { //Add event twice, with Wjets interchanged
						if (qW == 1) {
							LHCOEventsVector[1] = (TLorentzVector) *selectedJets[labelsReco[3]];
							LHCOEventsVector[2] = (TLorentzVector) *selectedJets[labelsReco[2]];
						}
						...
						for(int ii = 0; ii < 6; ii++){ //Loop over the different TLorentzVectors inside the array
							...
							if(ii == 0) muMinusLHCO << "  0             " << ProcessedLHCOEventsMuMinus << "        0 " << endl;  //Start of a new event
							muMinusLHCO << "  " << setprecision(1) << ii+1;
							muMinusLHCO << "    " << setprecision(1) << MadGraphId;
							cout.setf(ios::fixed,ios::floatfield);
							cout.precision(4);
							muMinusLHCO << "  " << fixed << showpoint << setprecision(4) << EtaValue;
							cout.setf(ios::fixed,ios::floatfield);
							cout.precision(4);
							muMinusLHCO << "  " << fixed << showpoint << setprecision(4) << PhiValue;
							muMinusLHCO << "  " << setprecision(4) << LHCOEventsVector[ii].Pt();
							cout.setf(ios::fixed,ios::floatfield);  //Add zero to obtain the asked number of digits
							cout.precision(4);
							muMinusLHCO << "  " << fixed << showpoint << setprecision(3) << MassValue;
							if(ii == 4) muMinusLHCO << "   " << setprecision(3) << "-1.000";
							else muMinusLHCO << "    0.00";
							if (ii == 0 || ii == 3) muMinusLHCO << "  1.000"; //Btag
							else muMinusLHCO << "  0.00";
							muMinusLHCO << "     0.00  0.00  0.00" << endl;
						}
					}
					ProcessedLHCOEventsMuMinus++;
					//Only 2000 events per MadWeight LHCO file
					if (ProcessedLHCOEventsMuMinus%1000 == 0) {
						muMinusLHCO.close();
						ostringstream ostr;
						ostr << MWSameDataSetNumberMuMinus;
						string newFileNumber = ostr.str();
						string strMuMinusNew = pathLHCO+"input_muMinus_"+thisDataSet+newFileNumber+".lhco";
						muMinusLHCO.open(strMuMinusNew.c_str());
						muMinusLHCO << "#</MGPGSCard> " << endl;
						muMinusLHCO << "#" << thisDataSet << endl;
						muMinusLHCO << "  #  typ      eta      phi       pt   jmas  ntrk  btag   had/em  dum1  dum2 " << endl;
						
						MWSameDataSetNumberMuMinus++;
					}
					
					//scaleFactor file
					muMinusScaleFactor << event->eventId() << "    ";
					if (correctEta) {
						cout.setf(ios::fixed,ios::floatfield);
						cout.precision(3);
						muMinusScaleFactor << fixed << setprecision(3) << scaleFactor;
					}
					else {
						muMinusScaleFactor << "0.000";
					}
					if (isCorrectMatch) {
						muMinusScaleFactor << "    " << "1.00" << endl;
					}
					else {
						muMinusScaleFactor << "    " << "0.00" << endl;
					}
				}
			}
    ...
    if (!useMassesAndResolutions && dataSetName.find("TTbarJets") == 0) {
      ...
      outfile->Close();
			...
			// Btag weights
			string filenameB = "PlotsForBtagWeights.root";
			
			cout << "INFO: Creating output file to store the plots for the btag weights " << filename << endl;
			
			TFile *outfileB = new TFile (filenameB.c_str(), "RECREATE");
			
			outfileB->cd();
			
			histo2D["BtaggedJets"]->Write();
			histo2D["BtaggedBJets"]->Write();
			histo2D["BtaggedCJets"]->Write();
			histo2D["BtaggedLightJets"]->Write();
			histo2D["TotalNofBJets"]->Write();
			histo2D["TotalNofCJets"]->Write();
			histo2D["TotalNofLightJets"]->Write();
      ...
      if(calculateTransferFunctions) {
				...
				for(int i = 0; i<9; i++){
					...
					if (TF_par1 && TF_par2 && TF_par3 && TF_par4 && TF_par5 && TF_par6) {
						...
						myTransferFunctions << "Constant broad gaussian & $a_{30}$ = " << TF_par3->GetParameter(0) << "$\\pm$" << TF_par3->GetParError(0) << " & $a_{31}$ = " << TF_par3->GetParameter(1) << "$\\pm$" << TF_par3->GetParError(1) << " & $a_{32}$ = " << TF_par3->GetParameter(2) << "$\\pm$" << TF_par3->GetParError(2) << "\\\\" << endl;
    ...
    if (jetTools) delete jetTools;
		muPlusLHCO.close();
		muMinusLHCO.close();
		muPlusLHCOextended.close();
		muMinusLHCOextended.close();
		muPlusScaleFactor.close();
		muMinusScaleFactor.close();
  ...
  //Matching
  cout << "Number of selected events: " << NofSelectedEvents << endl;
  cout << "Number of doubly btagged events: " << Nof2Btags << endl;
  cout << "In loop: " << unconditionalLoop << endl;
  cout << "In Chi2 loop: " << inChi2Loop << endl;
  cout << "Smallest Chi2 found: " << labelsChanged << endl;
  cout << "All4JetsMatched (MC): " << allMCJetsMatchedCondition << endl;
  cout << "Correctly matched: " << isCorrectlyMatched << endl;
  cout << "Not correctly matched: " << isNotCorrectlyMatched << endl;
  if (isCorrectlyMatched != 0 || isNotCorrectlyMatched != 0) {
    float percentageMatched = 100*(float)isCorrectlyMatched / (float)(isCorrectlyMatched + isNotCorrectlyMatched);
    cout << "This means that " << percentageMatched << "% is correctly matched." << endl;
  }
  cout << "Leptonic b matched: " << leptBMatched << endl;
  cout << "Hadronic b matched: " << hadrBMatched << endl;
  cout << "Jets from W matched: " << WjetsMatched << endl;
	cout << endl;
	cout << "Number of selected TTbar events: " << NofSelectedTTbarEvents << " of which " << NofTrueSemiMuTTbar << " are truly semi-mu at generator level." << endl;
	cout << "Number of selected Signal events: " << NofSelectedSignalEvents << " of which " << NofTrueSemiMuSignal << " are truly semi-mu at generator level." << endl;
	cout << endl;
  ...
  for(map<string,MultiSamplePlot*>::const_iterator it = MSPlot.begin(); it != MSPlot.end(); it++)
	{
		...
		temp->Draw(false, name, true, true, true, true, true, 25, false);
  ...
  string selectiontableMu = "SelectionTable_LightStop_SemiMu.tex";
  selecTableSemiMu.Write(selectiontableMu.c_str());
	
  // Do some special things with certain plots (normalise, BayesDivide, ... )
  if (verbose > 0)
