...
int main (int argc, char *argv[])
{
  ...
  cout << "*******************************************" << endl;
  cout << " Begining of the program for MVA Analysis ! " << endl;
  cout << "*******************************************" << endl;
  ...
  //xml file
  string xmlFileName ="../config/MVATrainer.xml";
  if (argc >= 2)
  ...
  MVAvars.push_back("pt");
  MVAvars.push_back("eta");
    
  MVAComputer* mva = new MVAComputer("Likelihood","TMVA_output.root","TestTrainer",MVAvars);
  ...
  for (unsigned int d = 0; d < datasets.size (); d++)
  {
	...
	mva->FillVar("pt",jet->Pt());
	mva->FillVar("eta",jet->Eta());

	std::cout << "MVA value " << mva->GetMVAValue() << endl;
