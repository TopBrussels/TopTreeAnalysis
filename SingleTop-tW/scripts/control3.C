...
void control3(int nsel, int mode = 0, bool silent = false){  
  ...
  sprintf(newRootFile,"results/crtt_correct_%dpb_%d.root", lumi, mode);
	...
	if (invMass){
	  if (metPt >= metCut || mode ==0){
	    ...
	    if (nJets != 0){
	      ...
	      double ptSysPx = lepton0.Px() + lepton1.Px() + metPx;
	      double ptSysPy = lepton0.Py() + lepton1.Py() +  metPy;
	      double ht = lepton0.Pt() + lepton1.Pt() + metPt; 
	      
	      for (int i = 0; i < ig; i++){
	        TLorentzVector jet(pxJet->at(indexJet[i]),pyJet->at(indexJet[i]), pzJet->at(indexJet[i]), eJet->at(indexJet[i]));
		ptSysPx += jet.Px();
		ptSysPy += jet.Py();
		ht += jet.Pt();
	      }
		...
		if (ht > htMin || mode !=0){
		  ...
		  if (nJets == 1 && nJetsBT == 1 && bTagged)histo_R->Fill(1, xlWeight);
		  if (nJets == 1 && nJetsBT == 2)  histo_R->Fill(2, xlWeight);
		  if (nJets == 1 && nJetsBT > 0)  histo_R->Fill(3, xlWeight);
		  if (nJets == 1 && nJetsBT > 1)  histo_R->Fill(4, xlWeight);
		  if (nJets == 2 && nJetsBT == 0)  histo_R->Fill(5, xlWeight);
		  if (nJets == 2 && nJetsBT == 1)  histo_R->Fill(6, xlWeight);
		  if (nJets == 2 && nJetsBT == 2)  histo_R->Fill(7, xlWeight);
		  if (nJets == 2 && nJetsBT > 0)  histo_R->Fill(8, xlWeight);
		  if (nJets == 2 && nJetsBT > 1)  histo_R->Fill(9, xlWeight);
		  if (nJets > 1 && nJetsBT == 0)  histo_R->Fill(10, xlWeight);
		  if (nJets > 1 && nJetsBT == 1)  histo_R->Fill(11, xlWeight);
		  if (nJets > 1 && nJetsBT == 2)  histo_R->Fill(12, xlWeight);
		  if (nJets > 1 && nJetsBT !=0 )  histo_R->Fill(13, xlWeight);
		  if (nJets > 1 && nJetsBT > 1 )  histo_R->Fill(14, xlWeight);
		  if (nJets == 3 && nJetsBT ==3 )  histo_R->Fill(15, xlWeight);
  ...
  if (!silent){ 
    cout << plotName ;
    cout << " ---> tt cr done" << endl;
    
    cout << "signal region: " <<  histo_R->GetBinContent(2) << ", 2j1t: " << histo_R->GetBinContent(7) << ", 2j2t: " <<  histo_R->GetBinContent(8)<< endl;
  }
  f_var.Write();
  f_var.Close();
