...
class MultiSamplePlot{
	...
	public:
		...
		void Draw(bool addRandomPseudoData = false, string label = string("CMSPlot"), bool mergeTT = false, bool mergeQCD = false, bool mergeW = false, bool mergeZ = false, bool mergeST = false, int scaleNPSignal = 1, bool addRatio = false);/** addRandomPseudoData will add a histo on top of the summed histograms which randomised according to a Poisson distribution. mergeTT/QCD/W/Z/ST will merge all the ttbar/QCD/W/Z/single-top together. */
		void Write(TFile* file, string label = string(""), bool savePNG = false, string pathPNG = string(""));
